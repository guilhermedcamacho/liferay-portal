...
public class LiferayTokenConverterTest {
	...
	public void testConvertMapSumsAllSources() {
		Assert.assertEquals(
			...
			LiferayTokenConverter.convert(
				Map.of(
					TokenSource.VERTEX_INPUT, 2340L, TokenSource.VERTEX_OUTPUT,
					525L)));
	}
	...
	public void testConvertUsingEmbeddingReturnsOneLRT() {
		Assert.assertEquals(
			1000L,
			LiferayTokenConverter.convert(TokenSource.EMBEDDING, 45000L));
	}
	...
	public void testConvertUsingModelArmorReturnsOneLRT() {
		Assert.assertEquals(
			1000L,
			LiferayTokenConverter.convert(TokenSource.MODEL_ARMOR, 90000L));
	}
	...
	public void testConvertUsingVertexInputReturnsOneLRT() {
		Assert.assertEquals(
			1000L,
			LiferayTokenConverter.convert(TokenSource.VERTEX_INPUT, 2340L));
	}
	...
	public void testConvertUsingVertexInputSmallValueKeepsFraction() {
		Assert.assertEquals(
			85L, LiferayTokenConverter.convert(TokenSource.VERTEX_INPUT, 200L));
	}
	...
	public void testConvertUsingVertexOutputReturnsOneLRT() {
		Assert.assertEquals(
			1000L,
			LiferayTokenConverter.convert(TokenSource.VERTEX_OUTPUT, 525L));
	}
