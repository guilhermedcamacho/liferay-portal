...
import com.liferay.object.rest.internal.jaxrs.exception.mapper.ExternalStorageExceptionMapper;
	...
	public synchronized List<ServiceRegistration<?>> deploy(
		...
		if (!_objectDefinitionsMap.containsKey(
			...
			Collections.addAll(
				...
				_bundleContext.registerService(
					ExceptionMapper.class, new ExternalStorageExceptionMapper(),
					HashMapDictionaryBuilder.<String, Object>put(
						"osgi.jaxrs.application.select",
						"(osgi.jaxrs.name=" + objectDefinition.getName() + ")"
					).put(
						...
						"osgi.jaxrs.name",
						objectDefinition.getName() +
							"ExternalStorageExceptionMapper"
					).build()),
				_bundleContext.registerService(
